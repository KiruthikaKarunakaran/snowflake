...
AGENT_DATABASE = "HACKATHON_DATASETS"
AGENT_SCHEMA = "TEAM_ATHENA"  
AGENT_NAME = "COMPANY_TICKER_AGENT"

SERVICES = [
    # Cortex Search Services (5) - Match Snowflake UI exactly (ALL UPPERCASE)
    {
        "name": "test Sentiment Search",
    ...
    {
        "name": "Athena Ticker whole Data",
        "location": "HACKATHON_DATASETS.TEAM_ATHENA.COMPANY_TICKER_DATA",
        "type": "analyst",
        "emoji": "🏢",
    },
    {
...
CUSTOM_INSTRUCTION = (
    """You are a helpful agent called InvestBuddy. Use the full conversation content when answering. 
       You can use Athena Ticker whole Data to answer the queries
       ...
       provide detail reasoning why you provided the suggestion.
       if the user ask for future price get for table stock_forecasted_data
       """

    
)

onboarding_q = '"What is the latest closing price of Snowflake stock? What is the market sentiments?"'
...
class ChatService:
    ...
    def handle_user_prompt(self, prompt: str):
        with st.spinner('InvestBuddy is thinking...'):
...
def main():
    st.set_page_config(page_title="InvestBuddy", page_icon="❄️", layout="wide")
    ...
    with st.sidebar:
        ...
        st.markdown("### ⚙️ InvestBuddy Settings")
        ...
        <b>InvestBuddy v{APP_VERSION}</b><br>
    ...
    if not state.messages:
        st.markdown(
            ...
            <div class="welcome-container">
                ...
                <div class="welcome-title">Welcome to InvestBuddy</div>
    ...
    # ── user input ──
    if prompt := st.chat_input("Warren is retiring, you are my new guru, InvestBuddy!!!"):
